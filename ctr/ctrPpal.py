    ...
    def add_datos_BDA_con_geometria(self,index,nomTablaSinEsquema,nomCapa,dicFormularios,objetosSeleccionados,esMulti=False,cargarDatosDeBda=False):
        ...
        if dicFormularios.get(clave)==None:
            ...
            if cargarDatosDeBda==False:
                dlg=ctrIntrodDatos_NGeom(self.oUtiles,nomTabla,[clave],mostrarBttNuevo=False,dicValoresAdd=None,geomWkt=geomWkt,esMulti=esMulti)
                dlg.exec_()#detiene la ejecucion hasta que se cierre el cuadro de dialogo
                dlg.set_dxf_featureId(dxf_featureId=dic.get("featureId"))
            else:
                dlg=ctrIntrodDatos_NGeom(self.oUtiles,nomTabla,[clave],mostrarBttNuevo=False,dicValoresAdd=None, geomWkt=None,esMulti=False)
    ...
    def add_linde(self,index):
        ...
        if self.dicLindes.get(clave)==None:
            if geomWkt==None:
                ...
                return
            nomTabla=self.oUtiles.get_nomTabla("lindes")
            dlgLinde=ctrIntrodDatos_NLinde(self.oUtiles,nomTabla,["lindes"],geomWkt=geomWkt)
...
#             
            ...
            if isinstance(img,Exception):
                ...
                return
            listaNomCampos=["id_trabajo","gid_linde","nom_arch","geom","archivo"]
            listaValores=[self.oUtiles.id_trabajo, gid_linde, nomImagen, geomWkt, img]
